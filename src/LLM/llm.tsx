...
export const LLM: React.FC<LLMProps> = ( { selectedIngredients } ) => {
  ...
  const SINGLE_INPUT = (ingredients: String) => {
    ...
    return(
      `Come up with as many popular, high protein recipes using a combination of some of these ingredients: <ingredients> ${ingredients} </ingredients>. You do not need to use every ingredient given, especially if the combination is not well-known. Return in this format: 
  ...
  const printRecipes = (recipes: Recipe[]) => {
    return recipes.map((recipe, index) => (
      <div key={index}>
        ...
        <p>Estimated Protein: {recipe["Estimated Protein"]}</p>
      </div>
    ));
  };
                  ...
                  <tr>
                    <td colSpan={2}>
                      <div>
                        ...
                        <p><strong>Estimated Protein:</strong> {recipe["Estimated Protein"]}</p>
